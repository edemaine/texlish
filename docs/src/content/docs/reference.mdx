...
`>>use` is shorthand for `\usepackage{...}`.
...
* `\begin{document}` before the first line starting with apparent text content.
...
For clarity, examples outside this section disable this feature
