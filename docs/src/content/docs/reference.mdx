...
<Example src={`
  ...
  This document uses Texlish 0.1.x+ syntax.
`}/>

Ranges should specify full `major.minor.patch` version numbers
(optionally with `-` prerelease), and can be prefixed by a comparison of
`<`, `<=`, `>`, `>=`, or `=` (the default meaning of a bare version).
Multiple comparisons (separated by whitespace) are combined with AND.
